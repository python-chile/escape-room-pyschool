---
title: "Habitación 5:"
include-in-header:
  text: |
    <script type="module">
    window.GLOBAL_URL = "https://raw.githubusercontent.com/Tony-Rome/pyscape/refs/heads/feature/estacion-espacial/quarto/rooms/sala_05.py";
    </script>
    <style>
    #quarto-sidebar, #quarto-sidebar-container { display: none !important; }
    </style>
---

<h2>
¡Estupendo! ya estás dominando las operaciones matemáticas.
</h2>

<center>
<img src="./sala_05.png" width="650">
</center>

<br/>

Una compuerta solo se abrirá si ingresas la suma de la secuencia desde el número 1 al número 1000.

<div class="alert alert-info">
Ocupa las funciónes sum(a) y range(b,c). Donde sum(a) suma el conjunto de números que se le envie y range(b,c) crea un rango de números.
</div>

::: {.callout-tip title="Rango que toma la función rage(b,c)" collapse="true"}
Pista de la habitación

Ten cuidado con la función `range(b,c)` ya que el rango es antes del valor de **c**, por lo que debes poner el siguiente valor si quieres incluir al valor de `c``.

Por ejemplo
```python 
suma = sum(range(5,10)) # range cuenta de 5 a 9.
```
:::

```{pyodide-python}
# Escribe tu respuesta aquí
respuesta = None
print(respuesta)

# Verifica la respuesta
import sala
sala.revisar(respuesta)
```